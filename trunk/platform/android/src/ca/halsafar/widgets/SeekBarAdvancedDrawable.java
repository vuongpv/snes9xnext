...
public class SeekBarAdvancedDrawable extends Drawable implements Runnable {
	...
	private static final String LOG_TAG = "AnimSeekBarDrawable";
	...
	private int mLevelDivisor;
	private String mTextUnit;
	
	public SeekBarAdvancedDrawable(Resources res, int startValue, int maxValue, int levelDivisor, String textUnit) {
		...
		mTextXScale = (startValue < maxValue / 2)? 1 : 0;
		mAnimation = new ScrollAnimation();
		mLevelDivisor = levelDivisor;
		mTextUnit = textUnit;
		
		//onLevelChange(startValue*levelDivisor);
	}
	...
	protected boolean onLevelChange(int level) {
		Log.d(LOG_TAG, "onLevelChange(" + level + ")");		
		mText = (level / mLevelDivisor) + " " + mTextUnit;
